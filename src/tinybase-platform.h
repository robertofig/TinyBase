...
external b32 WaitOnIoCompletion(file File, async* Async, usz* BytesTransferred, b32 Block);
...
|--- Return: 1 if successful, 0 in case of system error, 2 if [Src] does not exist, 3 if [Dst]
 |    directory does not exist, and 4 if [Dst] already existed (and was not meant to be
 |    overwritten). */
...
/* Changes location of file in [Src] to [Dst] in the file system. The entire directory tree in
 ...
 |  UTF16 on Windows, UTF8 on Linux).
|--- Return: 1 if successful, 0 in case of system error, 2 if [Src] does not exist, 3 if [Dst]
 |    directory does not exist, and 4 if [Dst] already existed. */

external b32 ChangeDirLocation(void* Src, void* Dst);

/* Same usage as ChangeFileLocation(), but with a directory as [Src].
|--- Return: 1 if successful, 0 in case of system error, 2 if [Src] does not exist, 3 if [Dst]
 |    directory tree does not exist, and 4 if [Dst] already exists. */
...
|--- Return: 1 if function fetched a new path into [Iter], or 0 if it has reached the end of
 |    the base path and there are no more files or dirs to read. */
